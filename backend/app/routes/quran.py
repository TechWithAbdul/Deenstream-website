...
# ─────────────────────────────────────────────────────────────────────────────
# ADD THIS CODE BLOCK TO YOUR EXISTING app/routes/quran.py
# Insert after the existing get_surah() route function.
# ─────────────────────────────────────────────────────────────────────────────

@router.get(
    "/juz/{juz_number}",
    summary="Fetch all ayahs in a Juz (1–30)",
    response_model=dict,
)
async def get_juz(
    juz_number: Annotated[int, Path(ge=1, le=30, description="Juz number (1–30)")],
) -> dict:
    """
    Returns all ayahs belonging to the given Juz.
    Upstream Ummah API endpoint: GET https://ummahapi.com/api/quran/juz/{juz_number}

    Each ayah object typically contains:
      surah_number, surah_name, surah_name_arabic, verse_number,
      text_arabic, text_en (english), text_ur (urdu), juz_number
    """
    client = get_ummah_client()
    try:
        response = await client.get(f"/quran/juz/{juz_number}")
        response.raise_for_status()
    except httpx.HTTPStatusError as exc:
        _raise_from_httpx(exc, f"quran/juz/{juz_number}")
    except httpx.RequestError as exc:
        logger.error("Network error fetching juz %d: %s", juz_number, exc)
        raise HTTPException(status_code=503, detail="Could not reach Ummah API. Network error.")

    try:
        payload = response.json()
    except Exception as exc:
        logger.error("Failed to parse juz %d JSON: %s", juz_number, exc)
        raise HTTPException(status_code=502, detail="Upstream returned non-JSON response.")

    if not isinstance(payload, dict):
        # Some APIs return bare list — wrap it
        if isinstance(payload, list):
            logger.debug("Juz %d returned as bare list; wrapping.", juz_number)
            return {"juz": juz_number, "ayahs": payload}
        logger.error("Unexpected payload type for juz %d: %s", juz_number, type(payload))
        raise HTTPException(status_code=502, detail="Unexpected upstream response format.")

    # Normalise: ensure ayahs key exists
    if "ayahs" not in payload and "verses" not in payload:
